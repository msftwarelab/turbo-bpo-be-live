...
func SaveIformTemp(ctx context.Context, pipelineId string, u models.UpdateIformTempInput, pipelinesRaw []*Pipeline, createdBy string) (string, error) {
	...
	res, err := DbCollections.IformTemps.InsertOne(ctx, newIformTemp)
	if err != nil {
		return "", err
	}
...
func UpdateIformTempStatus(ctx context.Context, pipelineId string, newStatus string) (bool, error) {
	...
	res, err := DbCollections.IformTemps.UpdateOne(ctx, filter, update)
	if err != nil {
		return false, err
	}
...
func UpdateIformTemp(ctx context.Context, pipelineId string, input models.UpdateIformTempInput, updatedBy string) (bool, error) {
	...
	iformRaw, err := GetIformTempByPipelineId(ctx, pipelineId)
	if err != nil || iformRaw == nil {
		_, err := SaveIformTemp(ctx, pipelineId, input, pipelinesRaw, updatedBy)
		if err != nil {
			return false, err
		}
		return true, nil
...
func GetIformTempByPipelineId(ctx context.Context, pipelineId string) (*models.IformTemp, error) {
	filter := bson.D{{"pipelineId", pipelineId}}
	a := &Iform{}
	//err := DbCollections.Iforms.FindOne(ctx, filter).Decode(a)
	resRaw := DbCollections.IformTemps.FindOne(ctx, filter)
	if resRaw.Err() != nil {
	...
	return a.ToIformTempModels(), nil
}

func SearchIformsTemp(ctx context.Context, offset, limit int, clientID, address *string) ([]*Iform, error) {
	...
	cur, err := DbCollections.IformTemps.Aggregate(ctx, pipe)
	if err != nil {
...
func GetIformTempByOrderNumber(ctx context.Context, orderNumber string) (*models.Iform, error) {
